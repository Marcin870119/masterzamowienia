...
function updateBanner() {
    ...
    // Fallback dla banerów w przypadku 404
    const fallbackBanner = 'https://via.placeholder.com/900x200?text=No+Banner';
    switch (activeTab) {
        case 'lithuania':
            bannerImage.src = 'https://raw.githubusercontent.com/Marcin870119/masterzamowienia/main/BANER LITWA.jpg' || fallbackBanner;
            bannerImage.style.display = 'block';
            break;
        case 'bulgaria':
            bannerImage.src = 'https://raw.githubusercontent.com/Marcin870119/masterzamowienia/main/BLUGARIA BANER.jpg' || fallbackBanner;
            bannerImage.style.display = 'block';
            break;
        case 'ukraine':
            bannerImage.src = 'https://raw.githubusercontent.com/Marcin870119/masterzamowienia/main/UKRAINA BANER 2.jpg' || fallbackBanner;
            bannerImage.style.display = 'block';
            break;
        case 'romania':
            bannerImage.src = 'https://raw.githubusercontent.com/Marcin870119/masterzamowienia/main/UKRAINA BANER 2.jpg' || fallbackBanner;
        ...
        case 'cart':
            ...
            break;
        default:
            bannerImage.src = 'https://i.postimg.cc/SNYKPTtw/Tekst-akapitu.jpg' || fallbackBanner;
            bannerImage.style.display = 'block';
    }
    // Obsługa błędu 404 dla banerów
    bannerImage.onerror = () => {
        console.warn(`Banner image not found for ${activeTab}, using fallback.`);
        bannerImage.src = fallbackBanner;
    };
}
...
function updateCart() {
    ...
    for (let country in productsData) {
        productsData[country].forEach((product, index) => {
            if (product.quantity > 0) {
                const imageUrl = `${country === 'lithuania' ? './zdjecia-litwa/' :
                    country === 'bulgaria' || country === 'romania' ? './zdjecia-bulgaria/' :
                    './zdjecia-ukraina/'}${product['INDEKS']}.jpg`;
                ...
                productElement.innerHTML = `
                    <img src="${imageUrl}" alt="Photo" style="position: relative; z-index: 0;" onerror="this.src='https://via.placeholder.com/100?text=No+Photo'">
...
function loadProducts(country) {
    ...
    // Placeholder dla testów - zastąp poprawnymi URL-ami
    let url = './produktyjson.json'; // Domyślny plik, jeśli żaden nie pasuje
    ...
    // Dane testowe w przypadku błędu 404
    const fallbackData = [
        {
            INDEKS: "test123",
            NAZWA: "Test Product",
    ...
    return fetch(url)
        .then(response => {
            ...
            if (!response.ok) {
                console.warn(`HTTP Error: ${response.status} for ${url}, using fallback data`);
                return fallbackData; // Używamy danych testowych w przypadku błędu
            }
        ...
        .then(data => {
            ...
            if (!data || data.length === 0) {
                console.warn(`No data received for ${country}, using fallback data`);
                data = fallbackData;
            }
            if (productsData[country].length > 0) {
            ...
            data.forEach((product, index) => {
                ...
                let imageUrl = '';
                if (country === 'lithuania') {
                    imageUrl = `./zdjecia-litwa/${product['INDEKS']}.jpg`;
                } else if (country === 'bulgaria' || country === 'romania') {
                    imageUrl = `./zdjecia-bulgaria/${product['INDEKS']}.jpg`;
                } else if (country === 'ukraine') {
                    imageUrl = `./zdjecia-ukraina/${product['INDEKS']}.jpg`;
                }
                ...
                img.onerror = () => {
                    console.warn(`Image not found for index ${product['INDEKS']}: ${imageUrl}`);
                    img.src = 'https://via.placeholder.com/100?text=No+Photo';
                };
        ...
        .catch(error => {
            ...
            productsData[country] = fallbackData.map((product, index) => ({ ...product, quantity: 0, dataset: { index } }));
            const productList = document.getElementById(`product-list-${country}`);
            if (productList) {
                productList.innerHTML = '<div style="padding: 20px; color: red;">Error loading products. Using fallback data.</div>';
                loadProducts(country); // Ponowne wywołanie z danymi testowymi
            }
        });
}
...
function applyFilters() {
    ...
    if (sortOrder && productsData[activeTab].length > 0) {
        products.sort((a, b) => {
            const rankA = parseInt(productsData[activeTab][a.dataset.index]?.Ranking || '0', 10) || 0;
            const rankB = parseInt(productsData[activeTab][b.dataset.index]?.Ranking || '0', 10) || 0;
    ...
    products.forEach(product => {
        ...
        const productCategory = productsData[activeTab][productIndex]?.Kategoria || '';
        ...
        const normalizedProductCategory = normalizeString(productCategory);

        const nameWords = productName.split(/\s+/);
        const searchMatch = searchTerm === '' || searchTerm.split(/\s+/).every(term =>
            nameWords.some(word => word.includes(term)) || productCode.includes(term)
        );
        const categoryMatch = selectedCategory === '' || normalizedProductCategory === normalizedSelectedCategory;

        console.log("Filter Debug - Index:", productIndex, "Category:", productCategory, "Normalized Category:", normalizedProductCategory, "Selected:", selectedCategory, "Normalized Selected:", normalizedSelectedCategory, "Match:", categoryMatch);
