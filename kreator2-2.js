          ...
          if (isLarge) {
            if (imgSrc) {
              try {
                ...
                const imgX = x + (boxWidth * itemLayout.image.x);
                const imgY = y + (boxHeight * itemLayout.image.y);
                const imgW = boxWidth * itemLayout.image.w;
                const imgH = boxHeight * itemLayout.image.h;
                doc.addImage(imgSrc, imgSrc.includes('image/png') ? "PNG" : "JPEG", imgX, imgY, imgW, imgH, undefined, save ? "SLOW" : "FAST");
              } catch (e) {
            ...
            const indexY = y + (boxHeight * itemLayout.index.y);
            const indexWidth = boxWidth * itemLayout.index.w;
            doc.text(`Indeks: ${p.indeks || '-'}`, indexX, indexY, { align: "center", maxWidth: indexWidth });
            ...
            if (showRanking && p.ranking) {
              ...
              const rankingY = y + (boxHeight * itemLayout.ranking.y);
              const rankingWidth = boxWidth * itemLayout.ranking.w;
              doc.text(`RANKING: ${p.ranking}`, rankingX, rankingY, { align: "center", maxWidth: rankingWidth });
            }
            ...
            if (showCena && p.cena) {
              ...
              const priceY = y + (boxHeight * itemLayout.price.y);
              const priceWidth = boxWidth * itemLayout.price.w;
              doc.text(`${showPriceLabel ? `${priceLabel}: ` : ''}${p.cena} ${currencySymbol}`, priceX, priceY, { align: "center", maxWidth: priceWidth });
            }
            ...
            if (showLogo && layout === "4" && logoSrc) {
              try {
                ...
                const logoX = x + (boxWidth * itemLayout.image.x);
                const logoY = y + (boxHeight * (itemLayout.image.y + itemLayout.image.h + 0.05));
                const logoW = 120;
                const logoH = 60;
            ...
            if (showEan && p.ean && p.barcode) {
              try {
                ...
                doc.addImage(p.barcode, "PNG", barcodeX, barcodeY, barcodeW, barcodeH, undefined, save ? "SLOW" : "FAST");
              } catch (e) {
            ...
            const nameY = y + (boxHeight * itemLayout.name.y);
            const nameWidth = boxWidth * itemLayout.name.w;
            doc.text(p.nazwa || "Brak nazwy", nameX, nameY, { maxWidth: nameWidth });
            ...
            const indexY = y + (boxHeight * itemLayout.index.y);
            const indexWidth = boxWidth * itemLayout.index.w;
            doc.text(`Indeks: ${p.indeks || 'Brak indeksu'}`, indexX, indexY, { maxWidth: indexWidth });
            ...
            if (showRanking && p.ranking) {
              ...
              const rankingY = y + (boxHeight * itemLayout.ranking.y);
              const rankingWidth = boxWidth * itemLayout.ranking.w;
              doc.text(`RANKING: ${p.ranking}`, rankingX, rankingY, { maxWidth: rankingWidth });
            }
            ...
            if (showCena && p.cena) {
              ...
              const priceY = y + (boxHeight * itemLayout.price.y);
              const priceWidth = boxWidth * itemLayout.price.w;
              doc.text(`${showPriceLabel ? `${priceLabel}: ` : ''}${p.cena} ${currencySymbol}`, priceX, priceY, { maxWidth: priceWidth });
            }
